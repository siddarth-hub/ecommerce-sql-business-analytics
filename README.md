...
**Scope note:** the dataset is a small, structured schema (10 tables, ~130 total rows) built specifically to exercise a wide range of SQL techniques end-to-end — not a live production dataset. The goal here is breadth and correctness across SQL concepts, not scale.
...
The original schema didn't include a manager/reporting-line column, but one of the business questions asks for an employee-manager hierarchy. Rather than skip it, I extended the schema with a self-referencing `Employees.ManagerID` column and populated it (each department's earliest-hired employee is modeled as its manager) so the self-join query runs against real data instead of staying hypothetical.
...
├── data/
│   └── seed_data.sql           # Seed data (INSERT statements)
├── queries/
